...
output "cache_bucket_name" {
  ...
  value       = module.context.enabled && local.s3_cache_enabled ? join("", aws_s3_bucket.cache_bucket.*.bucket) : "UNSET"
}

output "cache_bucket_arn" {
  description = "Cache S3 bucket ARN"
  value       = module.context.enabled && local.s3_cache_enabled ? join("", aws_s3_bucket.cache_bucket.*.arn) : "UNSET"
}
